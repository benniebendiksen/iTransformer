...
def apply_embedding_based_approach(model, train_data, val_data, test_data, device, top_n=50, ffn_epochs=50,
                                   ffn_lr=0.001):
    """
    Apply the embedding-based policy with multiple trading criteria

    This function uses the original model's predictions for test samples and decides
    whether to trade based on various policies using the model's performance on similar training samples.
    ...
    device : torch.device
        Device to run model on
    top_n : int
        Number of similar samples to use for analysis
    ffn_epochs : int
    ...
    --------
    dict
        Results of embedding-based approach with various trading policies
    """
    print("Applying embedding-based policy with multiple trading criteria...")
    ...
    for idx in range(len(test_data)):
        ...
        # Find similar samples from both train and val sets
        ...
        # Sort similar samples by similarity (descending) regardless of split type
        similar_samples.sort(key=lambda x: x[2], reverse=True)

        # Get embeddings and labels for similar samples
        similar_train_indices = []
        similar_val_indices = []

        for split, sim_idx, sim in similar_samples:
            if split == 'train':
                similar_train_indices.append((sim_idx, sim))
            elif split == 'val':
                similar_val_indices.append((sim_idx, sim))

        # Print similarity information
        print(f"Found {len(similar_train_indices)} similar training samples")
        print(f"Found {len(similar_val_indices)} similar validation samples")
        ...
        for train_idx, similarity_score in similar_train_indices:
            # Get training sample data
            train_batch_x, train_batch_y, train_batch_x_mark, train_batch_y_mark = train_data[train_idx]

            # Add batch dimension
            train_batch_x = torch.tensor(train_batch_x).unsqueeze(0).float().to(device)
            train_batch_x_mark = torch.tensor(train_batch_x_mark).unsqueeze(0).float().to(device)
            train_batch_y = torch.tensor(train_batch_y).unsqueeze(0).float().to(device)
            train_batch_y_mark = torch.tensor(train_batch_y_mark).unsqueeze(0).float().to(device)

            # Get model prediction for this similar sample
            dec_inp = torch.zeros_like(train_batch_y[:, -1:, :]).float().to(device)
            outputs = model(train_batch_x, train_batch_x_mark, dec_inp, train_batch_y_mark)
            similar_pred_prob = torch.sigmoid(outputs[:, -1, -1]).item()
            similar_pred = 1 if similar_pred_prob >= 0.5 else 0

            # Get true label
            similar_true = train_labels_array[train_idx]

            # Update confusion matrix
            if similar_pred == 1 and similar_true == 1:
                TP += 1
            elif similar_pred == 0 and similar_true == 0:
                TN += 1
            elif similar_pred == 1 and similar_true == 0:
                FP += 1
            elif similar_pred == 0 and similar_true == 1:
                FN += 1

            # Store predictions and true labels
            similar_preds.append(similar_pred)
            similar_trues.append(similar_true)

        # Process validation samples if any
        if val_data is not None and len(similar_val_indices) > 0:
            for val_idx, similarity_score in similar_val_indices:
                # Get validation sample data
                val_batch_x, val_batch_y, val_batch_x_mark, val_batch_y_mark = val_data[val_idx]

                # Add batch dimension
                val_batch_x = torch.tensor(val_batch_x).unsqueeze(0).float().to(device)
                val_batch_x_mark = torch.tensor(val_batch_x_mark).unsqueeze(0).float().to(device)
                val_batch_y = torch.tensor(val_batch_y).unsqueeze(0).float().to(device)
                val_batch_y_mark = torch.tensor(val_batch_y_mark).unsqueeze(0).float().to(device)

                # Get model prediction for this similar sample
                dec_inp = torch.zeros_like(val_batch_y[:, -1:, :]).float().to(device)
                outputs = model(val_batch_x, val_batch_x_mark, dec_inp, val_batch_y_mark)
                similar_pred_prob = torch.sigmoid(outputs[:, -1, -1]).item()
                similar_pred = 1 if similar_pred_prob >= 0.5 else 0

                # Get true label
                similar_true = val_batch_y[-1, -1].item()
        ...
        confusion_matrix_info = {
            ...
            'total_similar_samples': len(similar_train_indices)
        }
        confusion_matrices.append(confusion_matrix_info)

        print(f"Confusion matrix for test sample {idx}:")
        ...
        # Check if we have enough samples (combined from train and val)
        total_similar_samples = len(similar_train_indices) + len(similar_val_indices)
        if total_similar_samples < 10:
            print(f"Warning: Not enough similar samples found ({total_similar_samples}), using original prediction")
            embedding_preds.append(pred)
            embedding_probs.append(pred_prob)
            trade_decisions_original.append(False)
        ...
        print(
            f"Analyzing confusion matrix for {total_similar_samples} similar samples ({len(similar_train_indices)} train, {len(similar_val_indices)} val)...")

        # Use the original model's prediction for the test sample
        pred = original_preds[idx]
        pred_prob = original_probs[idx]
        ...
        # Policy 4: Confidence gap (at least 0.2 difference)
        confidence_gap = 0.35
    ...
    def calculate_policy_metrics(trade_mask):
        if np.sum(trade_mask) > 0:
            accuracy = accuracy_score(trues[trade_mask], embedding_preds[trade_mask])
            precision = precision_score(trues[trade_mask], embedding_preds[trade_mask], zero_division=0)
            recall = recall_score(trues[trade_mask], embedding_preds[trade_mask], zero_division=0)
            f1 = f1_score(trues[trade_mask], embedding_preds[trade_mask], zero_division=0)
            return {
                'accuracy': accuracy,
                'precision': precision,
                'recall': recall,
    ...
    total_accuracy = accuracy_score(trues, embedding_preds)
    ...
    print(f"\nEmbedding-based approach results:")
...
#         similar_val_indices = []
#
#         for split, idx, sim in similar_samples:
#             if split == 'train':
#                 similar_train_indices.append((idx, sim))
#             elif split == 'val':
#                 similar_val_indices.append((idx, sim))
#
#         # Print similarity information
#         print(f"Found {len(similar_train_indices)} similar training samples")
#         print(f"Found {len(similar_val_indices)} similar validation samples")
...
#         if len(similar_train_indices) < 10:
#             print(
#                 f"Warning: Not enough similar samples found ({len(similar_train_indices)}), using original prediction")
#             embedding_preds.append(pred)
#             embedding_probs.append(pred_prob)
#             trade_decisions_original.append(False)
#             trade_decisions_threshold.append(False)
...
#             continue
#
#         print(f"Analyzing confusion matrix for {len(similar_train_indices)} similar samples...")
#
#         # Use the original model's prediction for the test sample
#         pred = original_preds[idx]
#         pred_prob = original_probs[idx]
